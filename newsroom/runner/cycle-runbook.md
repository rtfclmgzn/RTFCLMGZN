...
1b. **Append your row to the ledger** — REQUIRED every cycle, including a cycle that publishes
   nothing. Append one row to `web/data/usage-log-current.js`, matching an existing row's fields
   exactly (it is a JS array literal, not JSON): a fresh sequential `u-NNNN` id past the highest in
   the file, a real UTC `ts`, `agent:"newsroom-cycle"`, `task_type:"publish"` or `"no-op"`,
   `article_id` set to the published article's id or `"system"`, an honest one-sentence
   `description` of what you did and why, and `measured:"estimated"` unless you have metered
   figures. Include `web/data/usage-log-current.js` in the `git add` below.
