(function () {
  ...
  document.getElementById("runDraftBtn").addEventListener("click", async function () { try { var task = await api("/api/autonomy/run", {method: "POST", body: {allow_publish: false, dry_run: false}}); openTask(task.task_id, "Research + draft cycle"); } catch (error) { toast(error.message, true); } });
