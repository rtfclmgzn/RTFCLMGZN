...
| A breaking-scan "retire stale cards" edit (2026-08-18) deleted a Buzz card's body but left its trailing `url:"...",}` line behind — a bare `key:"value"` is not legal as an array element in JS. `web/data/buzz.js` had a hard `SyntaxError` from that commit forward. **No check ever caught it, because no check ever read the file**: `site_guard.py` had zero functions calling `read_store()` on buzz.js, so the tolerant salvage parser that would have reported this immediately (it already does exactly this for every other store) was simply never asked. | The live site's `window.RTFC_BUZZ` never got defined from that commit onward — the Buzz page loaded empty (or stale, if a browser cached an older bundle) for the better part of a day, and it would have stayed that way indefinitely, since nothing was ever going to notice. | `check_buzz()` — calls `read_store()` on buzz.js (which feeds it straight into the existing salvage report) plus per-card required-field, duplicate-id, `heat` range, and date-format checks. Checked every other `web/data/*.js` file with `node --check` in the same pass; none of the others were affected. |
