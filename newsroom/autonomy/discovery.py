...
Return at most {int(limits['candidates_per_cycle'])} candidates. Prefer primary documents, regulator or company filings, official announcements, reputable wire reporting, and independent corroboration. Avoid rumors, recycled announcements, thin listicles, sensationalism, and stories whose material facts cannot be verified. Every candidate must include at least two real source leads when possible. Scores must reflect actual news value, not promotional language. Assign one of the canonical sections and a suitable persona ID from: {', '.join(self.registry.persona_ids())}.
"""
